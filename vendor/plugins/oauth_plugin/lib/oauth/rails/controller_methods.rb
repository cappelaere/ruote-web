...
module OAuth
  module Rails
    ...
    module ControllerMethods
      ...
      def oauthenticate
        logger.info "entering oauthenticate: #{headers.inspect}"
        verified=verify_oauth_signature 
        logger.info "verified=#{verified.to_s}"
        
        oauthenticated = verified && current_token.is_a?(::AccessToken)
      ...
      def oauth_required
        ...
        if oauthenticate
          ...
          if authorized?
            ...
            session[:user] = @current_client_application.user
            return true
          else
    ...
    private
      ...
      def verify_oauth_signature
        logger.info "* verify_oauth_signature"
        #logger.info "* Client Application request parameters: #{request.parameters}"
       
        begin
          valid = ClientApplication.verify_request(request) do |token, consumer_key|
            logger.info "verify_oauth_signature block #{consumer_key}"
